...
export default function ReviewSection({ schoolId, averageRating }: ReviewSectionProps) {
  ...
  // Fetch reviews with replies and like states
  useEffect(() => {
  ...
  const handleLikeReply = async (replyId: string) => {
    ...
    try {
      if (newLiked) {
        ...
        // UNLIKE → ONLY remove the like from reply_likes table
        // ❌ Do NOT insert or delete notifications
        await supabase
          .from("reply_likes")
          .delete()
        ...
        <div className="space-y-8">
          {reviews.map((review) => (
            <div
              key={review.id}
              id={`review-${review.id}`}
              className="bg-[#FDEDAA] rounded-3xl p-6 shadow-sm border border-yellow-100"
            >
              {/* Header */}
              ...
              <div className="flex items-center gap-6 mt-3">
                ...
                >
                  {review.liked ? (
                    <svg
                      className="w-5 h-5 fill-yellow-500 transition-transform hover:scale-110"
                      viewBox="0 0 24 24"
                    >
                    ...
                    <svg
                      className="w-5 h-5 stroke-gray-700 fill-none transition-transform hover:scale-110"
                      strokeWidth="2"
                      viewBox="0 0 24 24"
                    >
                      <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
              ...
              {review.replies.length > 0 && (
                <div className="mt-4 ml-6 border-l-2 border-yellow-300 pl-4 space-y-3">
                  {review.replies.map((reply) => (
                    <div
                      key={reply.id}
                      className="bg-[#FFF7D8] rounded-2xl p-3 border border-yellow-100"
                    >
                      ...
                      <div className="flex items-center gap-6 mt-2">
                        ...
                        >
                          {reply.liked ? (
                            <svg
                              className="w-4 h-4 fill-yellow-500 transition-transform hover:scale-110"
                              viewBox="0 0 24 24"
                            >
                              <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                            </svg>
                            ...
                            <svg
                              className="w-4 h-4 stroke-gray-600 fill-none transition-transform hover:scale-110"
                              strokeWidth="2"
                              viewBox="0 0 24 24"
                            >
                              <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                      ...
                      {/* Replying to a reply (ONLY shows under the clicked reply) */}
