  ...
  const handleLikeReply = async (replyId: string) => {
    ...
    // Check all replies including userOwnReview's replies
    const allReplies = [
      ...reviews.flatMap((r) => r.replies),
      ...(userOwnReview?.replies || [])
    ];
    
    const targetReply = allReplies.find((rep) => rep.id === replyId);
    if (!targetReply) return;

    // Find parent review from both reviews and userOwnReview
    const parentReview = reviews.find((r) => r.replies.some((rep) => rep.id === replyId)) 
      || (userOwnReview?.replies.some((rep) => rep.id === replyId) ? userOwnReview : null);
    ...
    try {
      ...
      // ALSO update userOwnReview state if the reply belongs to it
      if (userOwnReview?.replies.some((rep) => rep.id === replyId)) {
  ...
  const handleSendReply = async (targetId: string, type: "review" | "reply") => {
    ...
    if (type === "review") {
      ...
      // Check both reviews and userOwnReview for the parent review
      const parentReview = reviews.find((r) => r.replies.some((rep) => rep.id === targetId))
        || (userOwnReview?.replies.some((rep) => rep.id === targetId) ? userOwnReview : null);
    ...
    try {
      ...
      if (userOwnReview?.id === reviewIdForPayload) {
        setUserOwnReview({ ...userOwnReview, replies: [...userOwnReview.replies, newReply] });
      }
    } catch (err) {
            ...
            <div className="mb-8">
              <div className="bg-[#FFF9E3] rounded-2xl p-6 border border-yellow-200 shadow-sm">
                ...
                <div className="relative">
                  ...
                  {showDeleteDropdown && (
                    <div className="absolute top-full right-0 mt-2 bg-yellow-100 border border-yellow-300 rounded-xl shadow-lg p-3 z-50 w-[180px]">
                      <p className="text-gray-700 mb-3 text-center font-medium">Are you sure?</p>
                      <div className="flex gap-2 justify-center">
                        <button
                          onClick={() => setShowDeleteDropdown(false)}
                          className="px-4 py-1 rounded-full bg-gray-200 hover:bg-gray-300 text-sm"
                        >
                          No
                        </button>
                        <button
                          onClick={async () => {
                            if (!user || !userOwnReview) {
                              console.log("Missing user or review");
                              return;
                            }
                            
                            console.log("=== DELETE DEBUG ===");
                            console.log("User ID:", user.id);
                            console.log("Review ID:", userOwnReview.id);
                            console.log("Review user_id:", userOwnReview.user_id);
                            ...
                            try {
                              ...
                              console.log("Review exists check:", checkData);
                              ...
                              const { data: deleteData, error: deleteError } = await supabase
                                ...
                                .delete()
                                .eq("id", userOwnReview.id)
                                .eq("user_id", user.id) // Adding user_id check for RLS
                                .select();
                              ...
                              if (!deleteData || deleteData.length === 0) {
                                alert("No rows were deleted. Check your permissions.");
                                return;
                              }
                          ...
                          className="px-4 py-1 rounded-full bg-red-500 text-white hover:bg-red-600 text-sm"
                        >
                          Yes
                        </button>
                      </div>
                ...
                {!isEditing && (
                    <>
                      ...
                      {/* Like and Reply buttons for own review */}
                      <div className="flex items-center gap-6 mt-3">
                        <button
                          onClick={() => handleLikeReview(userOwnReview.id)}
                        ...
                        >
                          ...
                          <span className="text-gray-700 text-sm">{userOwnReview.likes ?? 0}</span>
                        </button>

                        <button
                          onClick={() => setReplyingTo({ id: userOwnReview.id, type: "review" })}
                          className="flex items-center gap-2 hover:opacity-80 transition"
                        >
                      ...
                      {/* Reply textarea for user's own review */}
                      {replyingTo?.type === "review" && replyingTo.id === userOwnReview.id && (
                        <div className="mt-3">
                          <textarea 
                          ...
                          <div className="mt-2 flex justify-end gap-2">
                            <button 
                              onClick={() => handleSendReply(userOwnReview.id, "review")} 
                ...
                {userOwnReview.replies.length > 0 && (
                  <div className="mt-4 ml-6 border-l-2 border-yellow-300 pl-4 space-y-3">
                    {userOwnReview.replies
                      ...
                      .map((reply) => {
                        const nestedReplies = userOwnReview.replies.filter(
                          (r) => r.parent_reply_id === reply.id
                        );

                        return (
                          <div key={reply.id}>
                            <div className="bg-[#FFF7D8] rounded-2xl p-3 border border-yellow-100">
                            ...
                            {nestedReplies.length > 0 && (
                              <div className="ml-8 mt-2 space-y-2">
                                {nestedReplies.map((nestedReply) => (
                                  <div key={nestedReply.id} className="bg-[#FFF7D8] rounded-2xl p-3 border border-yellow-100">
                                    <p className="text-xs text-gray-400 mb-1">↳ Replying to: "{reply.text.substring(0, 40)}{reply.text.length > 40 ? '...' : ''}"</p>
                                    
                                    <div className="flex items-center gap-2 mb-1">
                                      <ProfileAvatar userId={nestedReply.user_id} username={nestedReply.username} size={25} />
                                      <p className="font-semibold text-xs">{nestedReply.username}</p>
                                    </div>

                                    <p className="text-sm text-gray-600">{nestedReply.text}</p>

                                    <div className="flex items-center gap-6 mt-2">
                                      <button onClick={() => handleLikeReply(nestedReply.id)} className="flex items-center gap-1 hover:opacity-80 transition">
                                        {nestedReply.liked ? (
                                          <svg className="w-4 h-4 fill-yellow-500 transition-transform hover:scale-110" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>
                                        ) : (
                                          <svg className="w-4 h-4 stroke-gray-600 fill-none transition-transform hover:scale-110" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>
                                        )}
                                        <span className="text-xs text-gray-600">{nestedReply.likes}</span>
                                      </button>
                                    </div>
                                  </div>
                                ))}
          ...
          <div className="space-y-8">
              ...
              <div className="flex items-center gap-6 mt-3">
                ...
                >
                  ...
                  {/* Reply textarea for main review */}
              ...
              {/* Replies */}
                {review.replies?.length > 0 && (
                  <div className="mt-4 ml-6 border-l-2 border-yellow-300 pl-4 space-y-3">
                    {review.replies
                      .filter((reply) => reply && reply.id && !reply.parent_reply_id) // Only show top-level replies
                      .map((reply) => {
                        // Find nested replies (replies to this reply)
                        const nestedReplies = review.replies.filter(
                          (r) => r && r.id && r.parent_reply_id === reply.id
                        );
                        ...
                        return (
                          <div key={reply.id}>
                            <div className="bg-[#FFF7D8] rounded-2xl p-3 border border-yellow-100">
                              ...
                              <div className="flex items-center gap-6 mt-2">
                                ...
                                <button 
                                  ...
                                  className="text-sm text-gray-600 hover:opacity-80"
                                >
                                  Reply
                                </button>
                              </div>
                              ...
                              {replyingTo?.type === "reply" && replyingTo.id === reply.id && (
                                <div className="mt-3">
                                  ...
                                  <div className="mt-2 flex justify-end gap-2">
                                    <button 
                                      onClick={() => handleSendReply(reply.id, "reply")} 
                                      className="px-4 py-1 bg-yellow-400 rounded-full font-semibold hover:bg-yellow-500"
                                    >
                                      Send
                                    </button>
                            ...
                            {nestedReplies.length > 0 && (
                              <div className="ml-8 mt-2 space-y-2">
                                {nestedReplies.map((nestedReply) => (
                                  <div key={nestedReply.id} className="bg-[#FFFAEB] rounded-xl p-3 border border-yellow-50">
                                    <p className="text-xs text-gray-400 mb-2 italic">
                                      ↳ Replying to: "{reply.text && reply.text.length > 40 ? reply.text.substring(0, 40) + '...' : reply.text}"
                                    </p>
                                    
                                    <div className="flex items-center gap-2 mb-1">
                                      <ProfileAvatar userId={nestedReply.user_id} username={nestedReply.username} size={25} />
                                      <p className="font-semibold text-xs">{nestedReply.username ?? "Anonymous"}</p>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-6 mt-2">
                                      <button onClick={() => handleLikeReply(nestedReply.id)} className="flex items-center gap-1 hover:opacity-80 transition">
                                        ...
                                        <span className="text-xs text-gray-600">{nestedReply.likes ?? 0}</span>
                                      </button>
                                    </div>
                                  </div>
                                ))}
